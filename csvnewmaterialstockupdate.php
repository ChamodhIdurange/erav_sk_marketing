...
$filename = "process/newmaterialstockreport.csv";
...
while (($line = fgetcsv($file2)) !== FALSE) {
    ...
    $sqlproduct = "SELECT `idtbl_material` FROM `tbl_material` WHERE `materialbarcode` = '$productcode'";
    $resultproduct = $conn->query($sqlproduct);
    $row = $resultproduct->fetch_assoc();
    $productid = $row['idtbl_material'];
    ...
    $insertstock="INSERT INTO `tbl_material_stock` (`batchqty`, `qty`, `insertdatetime`, `status`, `batchno`, `updatedatetime`, `tbl_user_idtbl_user`, `tbl_material_idtbl_material`) VALUES ('$qty', '$qty', '$updatedatetime', '1', '$batchNo', '$updatedatetime', '$userID', '$productid')";
